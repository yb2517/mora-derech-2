...
// 3.2, הערה בשורה). הוא אינו מציג דבר ואינו משמיע דבר: הוא מחזיר
// מה יש למסור, ומנוע הקול הוא CONN-02 בשלב 5.
//
 ...
 */
export function create({ repository, send, caller, clock = defaultNow(), now } = {}) {
  if (!repository) {
   ...
   * לא אישר.
   */
  async function deliver({ state, sessionId, stopId, item, maxWords, accuracy, rest = [] }) {
  ...
  const ACTIONS = {
    ...
    arrive: async ({ payload = {} }) => {
      ...
      return ok(await deliver({
        state, sessionId, stopId: payload.stop_id, item,
        maxWords: maxWords.value, accuracy: payload.accuracy, rest: items,
      }));
    },
      ...
      if (held.reason === 'crossing' && held.stop_id === payload.stop_id) {
        ...
        return ok({
          ...(await deliver({
            ...
            maxWords: maxWords.value, accuracy: held.accuracy, rest: held.rest ?? [],
          })),
          released: true,
        });
      }
