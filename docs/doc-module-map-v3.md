...
סטטוס: גרסה 3.11, מאושרת 18.09.2026. גרסה 1 אושרה 11.09.2026; גרסה 2 (DESIGN-01 והמיפוי שורה לשורה) לא הובאה לאישור בנפרד ונבלעה בגרסה 3, שאושרה 12.09.2026.
תאריך: 12.09.2026
...
| חוק | הכלל | מקור |
|---|---|---|
| BL-01 | אין שינוי מצב של פריט או מסלול בלי רשומת APPROVALS. כשל כתיבה מבטל את המעבר. הכשל נמדד מול ה-Repository, הגבול היחיד שהמודול רואה (פער 58, הוכרע 16.09.2026): דרייבר שמחזיק תור כתיבה מקבל את השורה בזיכרון ומתחייב שלא לאבד אותה עד שהאחסון קיבל אותה, ודחייה קבועה נרשמת ומדווחת | usecase-f-07 זרימה ב |
...
| BL-10 | יומן ההחלטות (APPROVALS) חוסם פעולה כשאינו נכתב; יומן האינטראקציות (INTERACTIONS) לעולם אינו חוסם את החוויה בשטח, מנסה שוב ומסמן partial_log. 'נכתב' פירושו התקבל ב-Repository (פער 58); הדרך מה-Repository אל האחסון היא של הדרייבר, לפי BL-01 | usecase-f-09 זרימה א |
...
**בחירת המסך בהרכבה** (פער 63, הוכרע 16.09.2026): נקודת הכניסה אחת, והיא מציגה מסך אחד לפי פרמטר screen בכתובת, שערכו הוא שם הפונה של המסך (screen-traveler, screen-veto, screen-content, screen-owner). בלי פרמטר היא מציגה את הרכבת הפיתוח, ארבעת המסכים יחד. הפרמטר בוחר מה מוצג ואינו מרשה דבר: מה כל מסך רשאי לבקש נקבע ברשימת המותר בלבד, כמו היום. אימות זהות אינו כאן: הוא נכנס לפי 2.1, שורת ADMIN_USERS.
...
01 חיבור לנתונים בקובץ אחד לכל דרייבר: אחסון הדפדפן בקובץ אחד, מחרוזת החיבור לענן בקובץ אחד, ואף קובץ מחוץ ל-CORE-04 אינו נוגע באף אחד מהם (פער 59, 16.09.2026); 02 קריאה לספק AI בקובץ אחד; 03 כל מסך פונה לכתובת אחת ומצהיר מי הוא; 04 כל בקשה רשומה עם request_id; 05 ערכים משתנים מטבלת ה-reference; 06 הוספת מסך יוצרת שורה ברשימת המותר; 07 קודי השגיאה זהים בקוד ובמפה; 08 אף מודול אינו קורא למודול אחר ישירות.
...
| 5 | ממשקי קלט ואוטומציה: CONN-01 עד CONN-03, AUTO-01, AUTO-02, TOOL-01, וחסימת הסוללה | הליכה מלאה בסימולטור; BL-15; BL-20 | ספק הענן (הכרעה 2, הוכרעה 16.09.2026: Supabase, decision-05). חלק א נמסר 15.09.2026; חלק ב נמסר ומוזג 16.09.2026: דרייבר הענן על Supabase, והכתובת לטלפון ב-GitHub Pages (פער 61, decision-05 גרסה 1.1) |
| 6 | GW-01 כתפר ריק עם חוזה קריאה אחד לדוגמה | מבחן מבנה 02 | אין |
| 7 | ניקוי נתוני הדגמה, הסרת פעולת האיפוס, הקורפוס לרשומות, אימות עוגנים, is_crossing ונקודות יציאה בשטח | L1 עד L4 עוברים על 19 הפריטים ונעילה ראשונה נרשמת, במערך הבדיקות על הטקסטים האמיתיים; הנעילה החיה היא פעולת אדם אחרי רישום ההסכם, והדוח מציין אם בוצעה או ממתינה (פער 68, הוכרע 17.09.2026) | נקודה שהיא גם צומת וגם תוכן (הכרעה פתוחה 4) |
