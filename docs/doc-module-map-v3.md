...
סטטוס: גרסה 3.8, מאושרת 15.09.2026. גרסה 1 אושרה 11.09.2026; גרסה 2 (DESIGN-01 והמיפוי שורה לשורה) לא הובאה לאישור בנפרד ונבלעה בגרסה 3, שאושרה 12.09.2026.
תאריך: 12.09.2026
...
CONN-03 מוזרק ל-AUTO-01, ו-TOOL-01 הוא מקור דגימות חלופי שמוזרק במקומו בפיתוח (שורותיו ברשימת המותר נוגעות ל-arrive ול-leave בלבד, ואינן נוגעות לדגימות). שיוך CONN-01 ו-CONN-02 למודולים שמשתמשים בהם נקבע בתוכנית שלב 5 לפי F-02 ו-F-04.

**ההשלכה על ההרכבה**: נקודת הכניסה היא שמרכיבה את המתאמים ומזריקה אותם, ולכן היא רשאית לייבא מ-/connectors/ בלבד. מודולי שירות, אוטומציה ושערים ממשיכים להיטען מטבלת המודולים ולא בייבוא.
...
| BE-05 | listAnchors | F-01 צעד 3 (פער B-35) | module-geofence |
